...
toList ::
  ListZipper a
  -> List a
toList (ListZipper l a r) = reverse (a :. l) ++ r
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f (ListZipper (x :. xs) a r) = (if f x then IsZ else findLeft f) $ ListZipper xs x (a :. r)
findLeft _ _ = IsNotZ
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f (ListZipper l a (x :. xs)) = (if f x then IsZ else findRight f) $ ListZipper (a :. l) x xs
findRight _ _ = IsNotZ
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop x = toOptional (moveLeft x) ?? end x
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop x = toOptional (moveRight x) ?? start x
...
moveLeftN' 0 = Right
moveLeftN' x = left (+1) . (P.>>= moveLeftN' (x - signum x)) . (?? Left (-1)) . (<$>) Right . toOptional . f
  where f = if x < 0 then moveRight else moveLeft

-- moveLeftN' x = asMaybeZipper (moveLeftN' (x + 1)) . moveRight
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n z = moveLeftN (length (lefts z) - n) z
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l a (x :. xs)) = end $ ListZipper (a :. l) x xs
end x = x
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper (x :. xs) a r) = start $ ListZipper xs x (a :. r)
start x = x
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lf f rf) (ListZipper l a r) = ListZipper (zipWith ($) lf l) (f a) (zipWith ($) rf r)
...
instance Extend ListZipper where
  (<<=) f z = ListZipper l (f z) r
    where l = fold moveLeft
          r = fold moveRight
          fold fun = unfoldr (\x -> (f . id &&& id) <$> toOptional (fun x)) z
...
instance Traversable ListZipper where
  traverse f (ListZipper l x r) = lift3 ListZipper (traverse f l) (f x) (traverse f r)
