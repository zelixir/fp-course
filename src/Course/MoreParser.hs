...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok o c a = between (charTok o) (is c) (tok a)
...
hex ::
  Parser Char
hex = chr <$> tryParseString readHex (thisMany 4 (satisfy isHexDigit))
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll = satisfy . (\l c -> all ($ c) l)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny = satisfy . (\l c -> any ($ c) l)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma o c a = betweenCharTok o c (sepby a $ charTok ',')
