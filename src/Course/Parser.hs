...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) (P a) (P b) = P (choice . a <*> b)
  where choice r@(Result _ _) _ = r
        choice _ r = r
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) f (P a) = P (flip onResult (\i a' -> parse (f a') i) . a)
...
satisfy ::
  ...
  -> Parser Char
satisfy f = P p
  where p Nil = UnexpectedEof
        p (x :. xs) = if f x then Result xs x else UnexpectedChar x
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany n p = sequenceA (replicate n p)
...
tryParseString :: (Chars -> Optional b) -> Parser Chars -> Parser b
tryParseString f p = p >>= (\x -> (pure <$> f x) ?? constantParser (UnexpectedString x))
